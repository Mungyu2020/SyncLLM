...
class Graph:
    ...
    # MGC: Syncircuit diffusion model train dataset을 위해 생성 
    def graph2graphml(self, design_name, cmd, folder_dir):
        ...
        for name, node in self.node_dict.items():
            width_val = int(node.width) if node.width is not None else 1
            type_val = str(node.type)
            
            G.add_node(name, type=type_val, width=width_val)
        ...
        filename = os.path.join(folder_dir, f"{design_name}_{cmd}.graphml")
        try:
            nx.write_graphml(G, filename)
            print(f"GraphML saved successfully: {filename}")
    ...
    # MGC: For debug - .pkl 에 저장된 거랑 show_graph한거랑 같은지 체크
    # show_graph는 edge가 없는 node는 출력 안하는 것 확인
    def check_isolated_nodes(self):
        connected_nodes = self.get_all_nodes2()
        
        isolated_nodes = []
        for name in self.node_dict.keys():
            if name not in connected_nodes:
                isolated_nodes.append(name)
        print(f"Total Nodes in node_dict : {len(self.node_dict)}")
        print(f"Nodes with Edges         : {len(connected_nodes)}")
        print(f"Isolated (No Edge) Nodes : {len(isolated_nodes)}")
        
        if len(isolated_nodes) > 0:
            print(f"Examples of isolated nodes: {isolated_nodes[:10]} ...")
    ...
    def show_graph(self, design_name, file_dir):
        ...
        print('Finish!\n')
        png_outfile = outfile_path + f"{design_name}.png"
        
        exit_code = os.system(f"dot -Tpng {outfile} -o {png_outfile}")
        
        if exit_code == 0:
            print(f"Successfully generated image: {png_outfile}")
        else:
            print("Error: Failed to generate PNG. Make sure Graphviz is installed.")